...
const hasLineOfSight = function (x1, y1, x2, y2, firewalls) {
    ...
    // pre-build a lookup of firewall tiles so the loop below doesn't have
    // to create a new closure (.some(...)) on every single iteration
    const wall_keys = {};

    firewalls.forEach(function (fw) {
        wall_keys[fw.x + "," + fw.y] = true;
    });

    // bounded by manhattan distance – guarantees termination, no infinite
    // loop
    ...
    while (step < max_steps) {
        ...
        if (!(cx === x1 && cy === y1) && wall_keys[cx + "," + cy]) {
...
const processRespawnQueue = function (game) {
    ...
    let live_units = game.units.slice();
    const still_waiting = [];
...
const getReachableTiles = function (unit, game) {
    ...
    const expandFrom = function (current) {
        ...
        dirs.forEach(function (n) {
            ...
            // firewalls and cores block the path entirely – can't pass
            // through them
            ...
            // units block passage too – can't walk through any agent
 ...
 * Uses BFS reachability – agents must route around firewalls, not just
 * avoid them. Slowed agents cannot move. Walking over a data-cache picks
 * it up automatically.
 * @param {number}    x
 * @param {number}    y
 * @param {GameState} game
...
const attackUnit = function (x, y, game) {
    ...
    if (weapon.type === "phishing") {
        ...
        if (stealable) {
            updated_attacker = addWeaponToInventory(
                updated_attacker,
                stealable.type
            );
            // phishing steals the weapon outright – the enemy loses it,
            // not just a copy
            updated_target_inventory = target.inventory.filter(
                function (w) {
    ...
    let updated_units;
    let updated_queue = game.respawn_queue.slice();
        ...
        updated_units = game.units.map(function (u) {
            if (u.id === target.id) {
                return merge(u, {
                    ...
                    inventory: updated_target_inventory
                });
            }
            if (u.id === attacker.id) {
    ...
    // emp area effect – slow every enemy agent within range, not just
    // the target
    if (weapon.type === "emp") {
        updated_units = updated_units.map(function (u) {
...
const spawnWeaponDrop = function (game, random_fn) {
    ...
    const type = types[Math.floor(rand() * types.length)]; // injected random

    // lookup built once, outside the retry loop below – avoids creating
    // a new closure on every attempt (jslint forbids functions in loops)
    const wall_keys = {};

    game.firewalls.forEach(function (f) {
        wall_keys[f.x + "," + f.y] = true;
    });
    ...
    while (blocked) {
        ...
        blocked = (
            Boolean(getUnitAtPosition(x, y, game)) ||
            Boolean(getCoreAtPosition(x, y, game)) ||
            Boolean(getWeaponAtPosition(x, y, game)) ||
            Boolean(wall_keys[x + "," + y])
        );
    }
