...
// added the visor rect so it looks more like a hacker character
...
const weaponBadgeColor = function (type) {
    ...
    if (type === "overclock") { return "#ffffff"; }  // white – stands out as a utility item
...
const renderTile = function (x, y, game, onCellClick) {
    ...
    // build a descriptive label so screen readers announce what's on the tile
    let aria_desc = "node " + x + "," + y;
    if (getFirewallAtPosition(x, y, game)) { aria_desc += ", firewall"; }
    if (getCoreAtPosition(x, y, game)) {
        const c = getCoreAtPosition(x, y, game);
        aria_desc += ", " + (c.owner === PLAYER_1 ? "player 1" : "player 2") + " server, " + c.hp + " HP";
    }
    if (getUnitAtPosition(x, y, game)) {
        const u = getUnitAtPosition(x, y, game);
        aria_desc += ", " + (u.owner === PLAYER_1 ? "agent 01" : "agent 02") + ", " + u.hp + " HP";
    }
    if (getWeaponAtPosition(x, y, game)) {
        const d = getWeaponAtPosition(x, y, game);
        aria_desc += ", data cache: " + d.type;
    }
...
const renderInventory = function (game, onWeaponClick) {
    ...
    selected_unit.inventory.forEach(function (weapon, index) {
        ...
        // overclock shows movement range instead of damage/attack range
        ...
        btn.innerHTML = (
            '<span class="inv-badge" style="background:' + badge_col + ';color:' + (weapon.type === "overclock" ? "#000" : "#000") + '">' +
